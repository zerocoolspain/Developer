...
if [ "${sistema}" == "SunOS" ]
    then
        ...
        if [ $? -eq 0 ]
            then 
                echo " UNDO Rotado password usuario ${2} en servidor ${1} -> OK"
                ok="Y"
            else
                echo " UNDO Rotado password usuario ${2} en servidor ${1} -> NOK con error ${$?}"
                ok="N"
        fi
...
if [ "${sistema}" == "SunOS" ]
    then
        ...
        echo "${FECHA};${3};${1};${2};${4};${5};UNDO_APU;Vodafone-IT"
fi
...
echo "FECHA;ID;SERVIDOR;USUARIO;APLICACION;ENTORNO;ACCION;DUMMY"
...
done 3< undo_rotado_password.txt
continua
clear
...
sed $'s/[^[:print:]\t]//g' undo_rotado_password.log | grep -E "COMPROBACION|Vodafone-IT" > undo_rotado_password.csv
perl -npi -e "s/Press ENTER to continue ...//g" undo_rotado_password.csv
...
cat undo_rotado_password.csv | awk ' NR != 1'| while read line
do
...
mv temporal_script.tmp  undo_rotado_password.csv