    ...
    then
        ...
        cat /tmp/listado_usuarios_tocar.txt | while read line
            do
                ...
                echo "Simulamos cambiar el usuario ${line} en la linea ${buscaren} del SHADOW para poner el campo ${campotocar} en ${days}"
            done
fi
        ...
        cat /tmp/listado_usuarios_tocar.txt | while read line
            do
                if [ "$(awk -v user="${line}" -F: '{if($1 == user) {print $3}}' /etc/shadow)" = "$days" ] 
                    then
	                    echo "Simulamos ROTAR PASS usuario ${line}"
                    else
	                    echo "Simulamos NO ROTAR PASS usuario ${line}"
                fi
            done
fi  
            ...
            do
                if [ "$(awk -v user="${line}" -F: '{if($1 == user) {print $3}}' /etc/shadow)" = "$days" ] 
                    then
	                    ...
	                    echo "Simulamos NO ROTAR PASS usuario ${line}"w
                fi
            done
fi  
